...
class dataUtils():
    ...
    def ReadAndDisplayInputImages(self, imageCategory, numImagesToDisplay):
        ...
        Parameters:
            1. imageCategory - The category of the image (benign/malignant).
        ...
        for img_path in glob.glob(imgDirectory)[:numImagesToDisplay]:
        ...
        for i, image in enumerate(images):
            plt.subplot(rows, columns, i + 1)  # Correct row and column calculations
            plt.imshow(image)
            plt.axis('off')
    ...
    def DisplayAnnotatedImages(self, df, numImagesToDisplay):
        ...
        Parameters:
            1. df - The dataset which contains the filenames and the corresponding labels.
            2. numImagesToDisplay - Total number of images to display.
        ...
        for img_path in df['filename'][:numImagesToDisplay]:
            # Extract the corresponding image label
            label = df.loc[df['filename'] == img_path, 'label'].values[0]
            labels.append(label)
            images.append(img.imread(img_path))
        ...
        plt.figure(figsize=(20, 10))
        columns = 5
        rows = (len(images) + columns - 1) // columns  # Correct row calculation using integer division

        for i, image in enumerate(images):
            plt.subplot(rows, columns, i + 1)  # Correct row and column calculation
            plt.imshow(image)
            plt.title(labels[i])
            plt.axis('off')
